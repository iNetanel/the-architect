...
- **Planning entrypoint sent an OpenCode-only agent name to every non-OpenCode provider** — `core/planner.py`'s primary planning entrypoint (the one that runs on every fresh plan / `--plan` / Infinite Loop `plan=True` iteration) decided whether to use OpenCode's named-agent routing (`--agent architect` + `.architect/architect.json`) based on `provider.supports_agents()` alone, and its Claude-Code-only prompt-prepending fallback was additionally gated by `isinstance(provider, ClaudeCodeProvider)`. Claude Code also returns `True` from `supports_agents()` (it supports `--agent`, just with its own built-in agent names — `claude`, `Explore`, `general-purpose`, `Plan`, `statusline-setup` — not OpenCode's `"architect"`), so every fresh planning pass against Claude Code invoked `claude --agent architect` and failed outright with `--agent 'architect' not found`, 100% reproducibly, exhausting all 3 planning retries and killing the run. Codex CLI and Gemini CLI were silently missing the architect role prompt entirely, since the old fallback only ever matched `ClaudeCodeProvider` by `isinstance`. The same unguarded pattern also existed in `core/intelligence.py`'s project-intelligence pass (`agent_override="intelligence"`).
- Introduced two shared, provider-agnostic helpers in `core/provider_setup.py` — `uses_architect_agent_routing()` (single source of truth for whether the architect/intelligence/reviewer meta-roles should use OpenCode's named-agent routing) and `prepend_provider_role_prompt()` (duck-typed prompt-prepending fallback via `getattr`, works with any provider implementing `get_<role>_prompt()`, not hardcoded to one provider class). `core/planner.py`, `core/intelligence.py`, and `core/circuit.py` (replan path) now all call these shared helpers instead of each re-deriving the same logic ad hoc; `core/retrospective.py`'s existing `_prepend_provider_prompt` now delegates to the shared implementation. This keeps the architect/intelligence/reviewer routing decision consistent across OpenCode, Claude Code, Codex CLI, Gemini CLI, and any future provider, from one place.
- **Pre-planning live health-check probe had the same bug** — `cli.py`'s `_check_provider_update_before_model_work()` (runs a tiny live provider probe immediately before every planning/learning pass, including every Infinite Loop iteration) built its probe's `agent_override`/`config_override` from `provider.supports_agents()` alone, so it also sent `--agent architect` to Claude Code, Codex CLI, and Gemini CLI — failing the health probe with `--agent 'architect' not found` even when the provider was perfectly healthy. Now gated by the shared `uses_architect_agent_routing()` helper, same as every other meta-role call site.
- Also generalized a related, narrower agnosticism gap in `cli.py`: the `standalone_mode` OpenRouter-compatibility guard (clears a stale OpenRouter model ID left over from a previous OpenCode/free-mode run) was gated by `isinstance(provider, ClaudeCodeProvider)`, silently skipping the same protection for Codex CLI and Gemini CLI. Now gated by the existing `provider.supports_free_tier()` capability check, which is `False` for every non-OpenCode provider.
- Audited every other `supports_agents()` call site in the codebase (CLI execution-agent picker, TUI pre-run screens, `provider_health.py`, `runner.py`) — all remaining occurrences are legitimate execution-agent selection (a different, correct feature) and were left unchanged.
